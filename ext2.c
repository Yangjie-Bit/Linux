...
int main(void){
  ...
  // rb+ 读写打开一个二进制文件, 允许读和写
    ...
    printf("Please wait a minute!\n");
	  create_fileSystem();
    printf("create_fileSystem success!\n");
	}
	
	pf = fopen("file.img", "rb+");
  printf("fopen file.img success, i will operate it!\n");
	last_inode_bit=1;  
	last_block_bit=0; 
	read_super_block();
    ...
    // printf("my [root@localhost /]# joint success!\n");
    ...
    */
			...
			printf("Can't find this command, please try again!\n");
		}
	}
	return 0;
}
...
void rmdir(char temp[9]){
	...
	if(!strcmp(temp, "..") || !strcmp(temp, ".")) {
		printf("This directory . and .. are not allowed to be deleted!\n");
		return;
	}
...
void help(){
    ...
    printf("      * 07.show date now : date                  15.modify mode       : chmod + mode          *\n");
...
void date(){
	char date_time[128];
	time_t t = time(0); //获取当前系统的时间 
	strftime(date_time, sizeof(date_time), "%Y-%m-%d %H:%M:%S %A", localtime(&t));
	printf("%s\n", date_time);
}
